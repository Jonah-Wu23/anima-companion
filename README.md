...
二次元情感陪伴助手（Web/PWA）：文字聊天、语音聊天、多角色切换、3D 角色互动、换装、回忆相册。
...
### 多角色系统
- 角色切换：在同一应用内切换不同角色，保持一致交互流程
- 独立人格：每个角色可配置独立提示词、记忆与语音参数
- 资源映射：按角色加载对应 3D 模型、动作与服装资源
- 快速扩展：通过 `configs/` 新增角色配置并接入资源
...
- **相册图片**：崩坏星穹铁道与洛天依B站官方
- **洛天依音源**：张天潇（怎样使洛天依说话 BV1744y1W7Hn）
- **动作提供方**：ot0510_standbypack（作者：オトカム / Otokam_0510）、背景キャラ用ループ会話モーション（作者：Spinach）、土下座のモーション（作者：相上岡菊結構）、腕組みIA.モーション+（作者：NaNaSi）、ミニモーション集（作者：智沙都）
