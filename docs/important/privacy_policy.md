...
感谢您信任并使用“二次元情感陪伴助手”（以下简称“本应用”）。本应用是由 **吴宗河 (Jonah Wu)**（以下简称“开发者”或“我们”）开发的开源二次元陪伴项目。
...
* **使用场景**：当您使用“语音输入（按键/VAD）”功能与角色对话时。
* **收集内容**：您的实时语音录音数据。
...
* **使用目的**：用于通过大语言模型（LLM）生成角色扮演的回复内容，以及维护上下文记忆（Context）。
...
* **收集内容**：为了区分用户及提供VIP鉴权服务，我们可能会收集您的用户名、加密后的密码哈希、IP地址以及浏览器User-Agent信息。
...
2. **个性化设置**：您的偏好设置（如：是否开启自动播放语音、背景音乐音量、减弱动画效果等）将保存在您的浏览器本地。
3. **清除方式**：您可以随时通过浏览器设置清除 Cookie 和本地缓存，但这将导致您退出登录并重置设置。
...
1. **大语言模型服务商（LLM API）**：
为了实现智能对话，您的文本输入需要发送至 LLM 服务提供商（如 OpenAI、DeepSeek 或您自行配置的本地模型）。该过程仅用于生成回复，且受相关服务商隐私政策约束。
2. **开源与代码托管**：
...
1. **技术手段**：我们使用 HTTPS 加密协议传输您的数据，防止数据在传输过程中被窃听。后端的密码存储采用强哈希算法加密。
2. **数据隔离**：不同用户的对话数据在逻辑上是隔离的，普通用户无法访问他人的相册或聊天记录。
3. **安全局限性**：尽管我们采取了保护措施，但由于互联网技术的限制及可能存在的恶意攻击，我们无法保证信息百分之百安全。若发生数据泄露，我们将及时公告并采取补救措施。
...
3. 我们不会有意收集未成年人的个人信息。如发现未成年人在未获监护人同意的情况下注册使用了本服务，我们将设法尽快删除相关数据。
...
1. **麦克风权限**：
* **目的**：用于“按键说话”和“VAD（自动语音检测）”功能，录制语音以进行识别。
...
随着项目功能的迭代（如新增功能、架构调整），本隐私政策可能会进行更新。
